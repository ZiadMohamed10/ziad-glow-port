    ...
    delay: index * 0.2
      ...
      <div className="relative p-6 rounded-2xl bg-card border-2 border-border hover:border-primary transition-all duration-500 hover:scale-105 overflow-hidden">
        ...
        <div className="grid grid-cols-5 gap-6 justify-items-center px-2">
          {skills.map(skill => <motion.div key={skill.name} whileHover={{
          scale: 1.12,
          y: -5
              ...
              <div className="flex flex-col items-center gap-2">
                ...
                <span className="text-xs text-center opacity-0 group-hover/icon:opacity-100 transition-opacity duration-200 absolute -bottom-8 left-1/2 -translate-x-1/2 whitespace-nowrap bg-popover rounded z-10 mx-0 px-0 py-[22px]">
                  {skill.name}
                </span>
              </div>
            </motion.div>)}
  ...
  return <section id="skills" className="py-20 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {skillsData.map((category, index) => <SkillCard key={category.title} {...category} index={index} />)}
        </div>
      </div>
    </section>;
};